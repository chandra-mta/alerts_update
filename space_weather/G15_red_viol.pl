...
$infile="/data/mta4/space_weather/G15returned";
  ...
  if ($time_diff < 3000 && $p2 > $P2_lim) {
    ...
    if (!-s $p2r_lockfile) { # if an alert hasn't been sent, send one
      if ($p2_wait == 5) {
        ...
        print OUT "A Radiation violation of GOES 15 P2 (4-9 MeV) has occurred indicating a probable EPHIN P4GM trip.\n";
  ...
  if ($time_diff < 3000 && $p5 > $P5_lim) {
    ...
    if (!-s $p5r_lockfile) { # if an alert hasn't been sent, send one
      if ($p5_wait == 5) {
        ...
        print OUT "A Radiation violation of GOES 15 P5 (40-80 MeV) has occurred indicating a probable EPHIN P41GM trip.\n";
        print OUT "Telecon now on 1-877-521-0441 111165#\n";
        printf OUT " Value: %6.2f p/cm2-s-sr-MeV\n",$p5;
        printf OUT " Limit: %6.2f (temp limit for test)\n", $P5_lim;
        close OUT;
